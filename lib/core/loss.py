...
class HeatmapLoss(nn.Module):
    """[the heatmaploss]
    input pred/gt/mask, then compute and return the loss
    """
    def __init__(self):
        super().__init__()
...
class MultiLossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_joints = cfg.MODEL.NUM_JOINTS # ! 14
        self.num_stages = cfg.LOSS.NUM_STAGES # ! 2

        # ! one or two HeatmapLoss
        self.heatmaps_loss = \
            nn.ModuleList(
                [
                    HeatmapLoss()
    ...
    def forward(self, outputs, heatmaps, masks, joints):
        ...
        for idx in range(len(outputs)):
            ...
            if self.ae_loss[idx]:
                ...
                batch_size = tags_pred.size()[0]
                tags_pred = tags_pred.contiguous().view(batch_size, -1, 1) # ! 输出tag热图拉长再计算
...
class DaeLossFactory():
    def __init__(self, cfg):
        super().__init__()

        self.up_pose_criterion = nn.MSELoss().cuda()
        self.up_orie_criterion = nn.SmoothL1Loss().cuda()
        self.down_pose_criterion = nn.MSELoss().cuda()
        self.down_orie_criterion = nn.SmoothL1Loss().cuda()
        self.up_to_down_orie_criterion = nn.SmoothL1Loss().cuda()
        self.down_to_up_orie_criterion = nn.SmoothL1Loss().cuda()

        
        self.up_pose_criterion_factor = 1
        self.up_orie_criterion_factor = 1
        self.down_pose_criterion_factor = 1
        self.down_orie_criterion_factor = 1
        self.up_to_down_orie_criterion_factor = 1
        self.down_to_up_orie_criterion_factor = 1


    def __call__(self, outputs, heatmaps, masks, daes):
        up_conf_target, down_conf_target = list(map(lambda x: x.cuda(non_blocking=True), np.split(heatmaps[0], [12], axis=1)))  # [9]
        up_orie_target, down_orie_target = list(map(lambda x: x.cuda(non_blocking=True), np.split(daes[0][0], [24], axis=1)))  # [18]
        up_orie_target_weight, down_orie_target_weight = list(map(lambda x: x.cuda(non_blocking=True), np.split(daes[0][1], [24], axis=1))) #[18]
        up_to_down_orie_target, down_to_up_orie_target = list(map(lambda x: x.cuda(non_blocking=True), np.split(daes[0][2], [2], axis=1)))
        up_to_down_orie_target_weight, down_to_up_orie_target_weight = list(map(lambda x: x.cuda(non_blocking=True), np.split(daes[0][3], [2], axis=1)))
        
        # up_conf_target, down_conf_target = np.split(heatmaps[0], [9], axis=1)
        # up_orie_target, down_orie_target = np.split(daes[0][0], [18], axis=1)
        # up_orie_target_weight, down_orie_target_weight = np.split(daes[0][1], [18], axis=1)
        # up_to_down_orie_target, down_to_up_orie_target = np.split(daes[0][2], [2], axis=1)
        # up_to_down_orie_target_weight, down_to_up_orie_target_weight = np.split(daes[0][3], [2], axis=1)
        
        up_conf_pred_list, up_orie_pred_list, \
            down_conf_pred_list, down_orie_pred_list, \
            up_to_down_orie_pred_list, down_to_up_orie_pred_list = outputs

        # up_conf_target = up_conf_target.cuda()

        up_conf_loss = self.up_pose_criterion(
            up_conf_pred_list[0], up_conf_target)  # up_conf_loss

        down_conf_loss = self.down_pose_criterion(
            down_conf_pred_list[0], down_conf_target)  # down_conf_loss

        up_orie_output = up_orie_pred_list[0]
        up_orie_output = torch.mul(up_orie_output, up_orie_target_weight)
        up_orie_loss = self.up_orie_criterion(
            up_orie_output, up_orie_target)  # up_orie_loss

        down_orie_output = down_orie_pred_list[0]
        down_orie_output = torch.mul(down_orie_output, down_orie_target_weight)
        down_orie_loss = self.down_orie_criterion(
            down_orie_output, down_orie_target)  # down_orie_loss

        up_to_down_orie_output = up_to_down_orie_pred_list[0]
        up_to_down_orie_output = torch.mul(up_to_down_orie_output, up_to_down_orie_target_weight)
        up_to_down_orie_loss = self.up_to_down_orie_criterion(up_to_down_orie_output, up_to_down_orie_target) # up_to_down_orie_loss

        down_to_up_orie_output = down_to_up_orie_pred_list[0]
        down_to_up_orie_output = torch.mul(down_to_up_orie_output, down_to_up_orie_target_weight)
        down_to_up_orie_loss = self.down_to_up_orie_criterion(down_to_up_orie_output, down_to_up_orie_target) # down_to_up_orie_loss

        return up_conf_loss*self.up_pose_criterion_factor, \
            down_conf_loss*self.down_pose_criterion_factor, \
            up_orie_loss*self.up_orie_criterion_factor, \
            down_orie_loss*self.down_orie_criterion_factor, \
            up_to_down_orie_loss*self.up_to_down_orie_criterion_factor, \
            down_to_up_orie_loss*self.down_to_up_orie_criterion_factor
