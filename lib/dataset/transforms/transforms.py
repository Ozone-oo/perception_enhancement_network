...
__init__:
    input: [transforms]
    deal:  init the [transforms]
__call__:
    input: [image, mask, joints]
    deal:  deal and return the [input] with transforms
"""
...
__call__:
    input: [image, mask, joints]
    deal:  return [to_tensor(image), mask, joints]
"""
...
__call__:
    input: [image, mask, joints]
    deal:  return [normalize(image), mask, joints]
"""
...
"""[RandomHorizontalFlip: class]
__init__:
    input: [flip_index, output_size, prob]
    deal:  init the parms
__call__:
    input: [image, mask, joints]
    deal:  return [inputs] after random_flip
"""
...
class RandomAffineTransform(object):
    def __init__(self,
        ...
        self.scale_type = scale_type # ! scale computed with long edge or short edge
